...
*/
DLL_EXPORT void BestStump(uint8** PosFtrsVec, uint8** NegFtrsVec, double* PosWt, double* NegWt, int NP, int NN, uint32* SampFtrsId, int SampNum, double prior, int nBins, int nThreads, double* err, uint8* thrs)
{
	int i, j;
	uint8 Thrs1, Thrs2;
	...
	for(i = 0; i < SampNum; i++)
	{
		...
		{
			if(MinErr1 >= 1 - prior) {thrs[i] = nBins - 1; err[i] = 1 - prior;}
			else {thrs[i] = Thrs1; err[i] = MinErr1;}
		}
	}
